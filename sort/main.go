...
const SORTTHREADS = REALCORES

const SHUFFLERTHREADS = CORES
...
func main() {
	...
	if ConnectToPeers {
		...
		for remoteHost, remoteIndex := range indexMap {
			...
			for i := 0; i < SORTTHREADS; i++ {
				go ListenRoutine(readDone, remoteIndex, hostname, ipMap, indexMap, ports, i, &threadSortCount, rbufRequest, rbufRequestResponse)
			}
		}
	...
	SortStart = time.Now()
	go ShuffleController(totalHosts, &threadSortCount, hostname, indexMap, writeTo, writeDone, readDone, rbufRequest, rbufRequestResponse)
	...
	for {
		//course syncronization
		//TODO syncronize with proper messages
		if time.Now().Sub(AbsStart) > (time.Second * SORTSTART) {
			break
		}
	}
    if false {
	...
	for {
		...
		case seg = <-readDone:
			...
			if seg.n == -1 {
				recBufLock.Lock()
				(*doneHosts)++
                //log.Printf("Total Hosts reported done %d",*doneHosts)
				recBufLock.Unlock()
			} else {
				...
				//rItems should be checked. The idea is that every set of
				//chunked keys is arriving is SORTBUFSIZE chunks so we only
				//need to look at the first ket to determine which thread it
				//goes to.
	...
	for i := 0; i < datalen; i++ {
		...
		if hostIndex[masterIndex] == SORTBUFSIZE {
			...
			if myIndex == sorteeHost {
				...
				//log.Printf("Writing %d %d", sorteeHost, sorteeThread)
...
func ListenRoutine(readDone chan Segment2,
	...
	for {
		...
		//before sending the buffer to the receiving thread.
            ...
            //if hostConnection == 1 && thisHostId == 1 {
            /*
                for i := 0; i < SORTTHREADS ;i ++ {
                    tLock[i].Lock()
                    log.Printf("thread %d, rec %d ",i,(*threadCounter)[i])
                    tLock[i].Unlock()
                }
            */
            //}
...
func detectTCPClose(c net.Conn) bool {
	...
	if err == nil {
        if !shuffleStarted {
		    ...
		    c.SetReadDeadline(time.Now().Add(time.Second * READTIMEOUT2))
        }
